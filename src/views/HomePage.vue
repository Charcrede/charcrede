...
<template>
  ...
  <div class="lg:mt-64 xs:mt-32 lg:px-56 xs:px-8 relative">
    ...
    <section class="text-white">
      <h2 class="lg:text-[3rem] xs:text-[1.5rem] text-center font-semibold font-clash lg:mb-8 xs:mb-4">Projet récent</h2>
      <figure class="lg:flex xs:block gap-8 items-center justify-between">
        <div class="lg:w-2/5 xs:w-full text-left">
          ...
          <div class="relative group w-fit lg:my-4  xs:mb-2 lg:text-2xl xs:text-xl xs:left-3/4">
            <div class="-z-10 absolute top-0 bottom-0 left-0 w-0 group-hover:w-full bg-white duration-300"></div>
            <a href="https://anime-quizzer.vercel.app/" target="_blanck" class="block font-bold group-hover:text-black duration-300 p-2 px-4">Visiter  </a>
          </div>
        </div>
        <div class="lg:w-3/5 xs:w-full group relative">
          <swiper :slides-per-view="1" :space-between="50" @swiper="onSwiper" @slideChange="onSlideChange" class="rounded-2xl w-full">
            <swiper-slide v-for="i in 3" :key="i">
              <img :src="require(`../assets/quizzer.png`)" alt="">
            </swiper-slide>
          </swiper>
          <div class="absolute top-0 bottom-0 left-0 right-0 bg-black bg-opacity-50 z-10 opacity-0 group-hover:opacity-100 duration-300 flex justify-center backdrop-blur-sm">
            <span class="flex justify-center items-center gap-8 text-[2rem] font-clash font-semibold text-[#aaa]">
